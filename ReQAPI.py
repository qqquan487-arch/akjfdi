import requests, json, base64, time, struct, datetime, re, urllib.parse, os
...
_CACHE_BD_VERSION = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "bdversion_cache.json"))
...
def bdversion(ver: str = None):
 if not ver:
  ver = storeApps("details?id=com.dts.freefireth") or "1.123.2"
 I="https://bdversion.ggbluefox.com/live/ver.php{}"
 II="?version=%s&lang=vi&device=android&region=VN" % ver
 try:
  if os.path.exists(_CACHE_BD_VERSION):
   with open(_CACHE_BD_VERSION, "r", encoding="utf-8") as f:
    cached = json.load(f)
    if isinstance(cached, dict) and "remote_version" in cached and "server_url" in cached:
     return cached
 except Exception: pass
 try:
  res=requests.get(I.format(II), timeout=10)
  res.raise_for_status()
  data = res.json()
  try:
   with open(_CACHE_BD_VERSION, "w", encoding="utf-8") as f:
    json.dump(data, f)
  except Exception: pass
  return data
 except Exception as e:
  fallback = {
   "remote_version": "1.123.2",
...
class APIClient:
 def __init__(self):
  ...
  host_header = urllib.parse.urlparse(self.base_url).netloc or "loginbp.ggblueshark.com"
  ...
  "Host": host_header, "ReleaseVersion": self.release_version
  })
 ...
 def MajorLogin(self):
  ...
  fields[90] = "Singapore"
  fields[94] = "KqsHT0qaTCGUXRYnJ0Rqk4rOvTBtqRFCqrxSLo/afYBAXyCA5v4zw5F/rWCSaZuZONmV1TMDDY0q0rZ4Kys1ITUFfGM=" if self.is_emulator else "KqsHT1y2dlDX0ywnP1LQ75AXqqV8YVvFC48pUhDlHSFPi7zihMoH4je/A9lW1Sa5OUKZngMdKfCwTE8lUtNlp7X97/w="
  fields[95] = 111111
  ...
  try:
   ...
   self._data.account_id = result["account_id"]
   self._data.server = result["server"]
   self._data.login_token = result["login_token"]
   self._data.base_url = result["base_url"]
   self._data.login_time = result["login_time"]
   self._data.key = result["key"]
   self._data.iv = result["iv"]
   return result
  except Exception as e:
 ...
 def GetLoginData(self):
      try:
        tokendec = gringay.tokendecode(self._data.login_token) or {}
        fields = {}
        fields[3]  = time.strftime("%Y-%m-%d %H:%M:%S")
        fields[7]  = self.client_version
        ext_type = tokendec.get("external_type")
        fields[23] = int(ext_type) if isinstance(ext_type, (int, str)) and str(ext_type).isdigit() else 0
        ext_id = tokendec.get("external_id")
        fields[29] = str(ext_id) if ext_id is not None else ""
        fields[4]  = "free fire"
        fields[5]  = 1
        fields[8]  = "Android OS 9 / API-28 (PQ3A.190605.03171033/3793265)"
        fields[9]  = "Handheld"
        ...
        fields[20] = "104.28.156.112"
        fields[21] = self.language
        fields[22] = "f1cd5406d38157cd6dab1389e022602d"
        fields[24] = "Handheld"
        fields[25] = "Xiaomi 2203121C"
        fields[26] = "SG"
        fields[41] = "MobiFone"
 ...
 def auth(self, access_token, is_emulator = False):
  try:
   ...
   if ":" in access_token:
    ...
    status = self.auth_guest_token(int(uid), password)
    if status != "ok":
     raise RuntimeError("guest token invalid or unavailable")
   else:
    status = self.auth_token_inspect(access_token)
    if status != "ok":
     raise RuntimeError("access token invalid")
   self.MajorLogin()
   self.GetLoginData()
